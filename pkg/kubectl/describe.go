...
// Kubernetes types.
func DescriberFor(kind string, c client.VersionInterface) (Describer, bool) {
	var f Describer
	var ok bool
	switch c.(type) {
	case *client.Client:
		f, ok = describerMap(c.(*client.Client))[kind]
	case *client.ExperimentalClient:
		f, ok = expDescriberMap(c.(*client.ExperimentalClient))[kind]
	}
	return f, ok
}
