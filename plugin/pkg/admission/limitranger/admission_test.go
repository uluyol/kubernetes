...
func TestLimitRangerIgnoresSubresource(t *testing.T) {
	...
	err := handler.Admit(admission.NewAttributesRecord(&testPod, "api", "Pod", limitRange.Namespace, "testPod", "pods", "", admission.Update, nil))
	if err == nil {
		t.Errorf("Expected an error since the pod did not specify resource limits in its update call")
	}

	err = handler.Admit(admission.NewAttributesRecord(&testPod, "api", "Pod", limitRange.Namespace, "testPod", "pods", "status", admission.Update, nil))
	if err != nil {
