...
func TestIgnoresNonCreate(t *testing.T) {
	...
	for _, op := range []admission.Operation{admission.Update, admission.Delete, admission.Connect} {
		attrs := admission.NewAttributesRecord(pod, "api", "Pod", "myns", "myname", string(api.ResourcePods), "", op, nil)
...
func TestIgnoresNonPodResource(t *testing.T) {
	pod := &api.Pod{}
	attrs := admission.NewAttributesRecord(pod, "api", "Pod", "myns", "myname", "CustomResource", "", admission.Create, nil)
...
func TestIgnoresNilObject(t *testing.T) {
	attrs := admission.NewAttributesRecord(nil, "api", "Pod", "myns", "myname", string(api.ResourcePods), "", admission.Create, nil)
	err := NewServiceAccount(nil).Admit(attrs)
	if err != nil {
...
func TestIgnoresNonPodObject(t *testing.T) {
	obj := &api.Namespace{}
	attrs := admission.NewAttributesRecord(obj, "api", "Pod", "myns", "myname", string(api.ResourcePods), "", admission.Create, nil)
	err := NewServiceAccount(nil).Admit(attrs)
	if err != nil {
...
func TestIgnoresMirrorPod(t *testing.T) {
	...
	attrs := admission.NewAttributesRecord(pod, "api", "Pod", "myns", "myname", string(api.ResourcePods), "", admission.Create, nil)
	err := NewServiceAccount(nil).Admit(attrs)
	if err != nil {
...
func TestAssignsDefaultServiceAccountAndToleratesMissingAPIToken(t *testing.T) {
	...
	pod := &api.Pod{}
	attrs := admission.NewAttributesRecord(pod, "api", "Pod", ns, "myname", string(api.ResourcePods), "", admission.Create, nil)
