...
func TestAdmissionIgnoresDelete(t *testing.T) {
	...
	err := handler.Admit(admission.NewAttributesRecord(nil, "api", "Pod", namespace, "name", "pods", "", admission.Delete, nil))
	if err != nil {
...
func TestAdmissionIgnoresSubresources(t *testing.T) {
	...
	err := handler.Admit(admission.NewAttributesRecord(newPod, "api", "Pod", newPod.Namespace, "123", "pods", "", admission.Create, nil))
	if err == nil {
		t.Errorf("Expected an error because the pod exceeded allowed quota")
	}

	err = handler.Admit(admission.NewAttributesRecord(newPod, "api", "Pod", newPod.Namespace, "123", "pods", "subresource", admission.Create, nil))
	if err != nil {
...
func TestIncrementUsagePods(t *testing.T) {
	...
	dirty, err := IncrementUsage(admission.NewAttributesRecord(&api.Pod{}, "api", "Pod", namespace, "name", "pods", "", admission.Create, nil), status, client)
	if err != nil {
...
func TestIncrementUsageMemory(t *testing.T) {
	...
	dirty, err := IncrementUsage(admission.NewAttributesRecord(newPod, "api", "Pod", namespace, "name", "pods", "", admission.Create, nil), status, client)
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	if !dirty {
...
func TestExceedUsageMemory(t *testing.T) {
	...
	_, err := IncrementUsage(admission.NewAttributesRecord(newPod, "api", "Pod", namespace, "name", "pods", "", admission.Create, nil), status, client)
	if err == nil {
...
func TestExceedUsageCPU(t *testing.T) {
	...
	_, err := IncrementUsage(admission.NewAttributesRecord(newPod, "api", "Pod", namespace, newPod.Name, "pods", "", admission.Create, nil), status, client)
	if err == nil {
...
func TestExceedUsagePods(t *testing.T) {
	...
	status.Used[r] = resource.MustParse("1")
	_, err := IncrementUsage(admission.NewAttributesRecord(&api.Pod{}, "api", "Pod", namespace, "name", "pods", "", admission.Create, nil), status, client)
	if err == nil {
...
func TestIncrementUsageServices(t *testing.T) {
	...
	status.Hard[r] = resource.MustParse("2")
	status.Used[r] = resource.MustParse("1")
	dirty, err := IncrementUsage(admission.NewAttributesRecord(&api.Service{}, "api", "Service", namespace, "name", "services", "", admission.Create, nil), status, client)
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	if !dirty {
...
func TestExceedUsageServices(t *testing.T) {
	...
	status.Hard[r] = resource.MustParse("1")
	status.Used[r] = resource.MustParse("1")
	_, err := IncrementUsage(admission.NewAttributesRecord(&api.Service{}, "api", "Service", namespace, "name", "services", "", admission.Create, nil), status, client)
	if err == nil {
...
func TestIncrementUsageReplicationControllers(t *testing.T) {
	...
	status.Hard[r] = resource.MustParse("2")
	status.Used[r] = resource.MustParse("1")
	dirty, err := IncrementUsage(admission.NewAttributesRecord(&api.ReplicationController{}, "api", "ReplicationController", namespace, "name", "replicationControllers", "", admission.Create, nil), status, client)
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	if !dirty {
...
func TestExceedUsageReplicationControllers(t *testing.T) {
	...
	status.Hard[r] = resource.MustParse("1")
	status.Used[r] = resource.MustParse("1")
	_, err := IncrementUsage(admission.NewAttributesRecord(&api.ReplicationController{}, "api", "ReplicationController", namespace, "name", "replicationControllers", "", admission.Create, nil), status, client)
	if err == nil {
...
func TestExceedUsageSecrets(t *testing.T) {
	...
	status.Hard[r] = resource.MustParse("1")
	status.Used[r] = resource.MustParse("1")
	_, err := IncrementUsage(admission.NewAttributesRecord(&api.Secret{}, "api", "Secret", namespace, "name", "secrets", "", admission.Create, nil), status, client)
	if err == nil {
		t.Errorf("Expected error for exceeding hard limits")
	}
}
...
func TestExceedUsagePersistentVolumeClaims(t *testing.T) {
	...
	status.Hard[r] = resource.MustParse("1")
	status.Used[r] = resource.MustParse("1")
	_, err := IncrementUsage(admission.NewAttributesRecord(&api.PersistentVolumeClaim{}, "api", "PersistentVolumeClaim", namespace, "name", "persistentVolumeClaims", "", admission.Create, nil), status, client)
	if err == nil {
		t.Errorf("Expected error for exceeding hard limits")
	}
}
